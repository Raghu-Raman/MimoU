...
def random_sampling(tensor_NHWC, n, indices=None):
    N, H, W, C = tf.convert_to_tensor(value=tensor_NHWC).shape.as_list()
    S = H * W
    ...
    indices_old = tf.random.uniform([n], 0, S, tf.int32) if indices is None else indices
...
def random_pooling(feats, output_1d_size=100):
    ...
    feats = [tf.convert_to_tensor(value=feats_i) for feats_i in feats]
