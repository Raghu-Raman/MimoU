...
class CSFlow:
    ...
    def create_using_L2(I_features, T_features, sigma = float(0.1), b = float(1.0)):
        ...
        with tf.compat.v1.name_scope('CS'):
            ...
            r_Ts = tf.reduce_sum(input_tensor=Tvecs * Tvecs, axis=2)
            r_Is = tf.reduce_sum(input_tensor=Ivecs * Ivecs, axis=2)
            raw_distances_list = []
            for i in range(sT[TensorAxis.N]):
                Ivec, Tvec, r_T, r_I = Ivecs[i], Tvecs[i], r_Ts[i], r_Is[i]
                A = Tvec @ tf.transpose(a=Ivec)
                cs_flow.A = A
                ...
                cs_shape[0] = 1
                dist = tf.reshape(tf.transpose(a=dist), cs_shape)
                # protecting against numerical problems, dist should be positive
                dist = tf.maximum(float(0.0), dist)
                # dist = tf.sqrt(dist)
                raw_distances_list += [dist]

            cs_flow.raw_distances = tf.convert_to_tensor(value=[tf.squeeze(raw_dist, axis=0) for raw_dist in raw_distances_list])
    ...
    def create_using_dotP(I_features, T_features, sigma = float(1.0), b = float(1.0)):
        ...
        with tf.compat.v1.name_scope('CS'):
            ...
            with tf.compat.v1.name_scope('TFeatures'):
                T_features = CSFlow.l2_normalize_channelwise(T_features)
            with tf.compat.v1.name_scope('IFeatures'):
                ...
                for i in range(N):
                    ...
                    cosine_dist_i = tf.nn.conv2d(input=I_features_i, filters=patches_HWCN_i, strides=[1, 1, 1, 1],
    ...
    def calc_relative_distances(self, axis=TensorAxis.C):
        epsilon = 1e-5
        div = tf.reduce_min(input_tensor=self.raw_distances, axis=axis, keepdims=True)
    ...
    def weighted_average_dist(self, axis = TensorAxis.C):
        ...
        return tf.reduce_sum(input_tensor=multiply, axis=axis, name='weightedDistPerPatch')
    ...
    def sum_normalize(cs, axis=TensorAxis.C):
        reduce_sum = tf.reduce_sum(input_tensor=cs, axis=axis, keepdims=True, name='sum')
    ...
    def center_by_T(self, T_features, I_features):
        ...
        self.meanT, self.varT = tf.nn.moments(
            x=T_features, axes=axes, name='TFeatures/moments')
        ...
        with tf.compat.v1.name_scope('TFeatures/centering'):
            self.T_features_centered = T_features - self.meanT
        with tf.compat.v1.name_scope('IFeatures/centering'):
    ...
    def l2_normalize_channelwise(features):
        norms = tf.norm(tensor=features, ord='euclidean', axis=TensorAxis.C, name='norm')
    ...
    def patch_decomposition(self, T_features):
        ...
        self.patches_NHWC = tf.reshape(
            ...
            shape=[-1, patch_size, patch_size, patches_as_depth_vectors.shape.as_list()[3]],
            name='patches_PHWC')
...
def CX_loss(T_features, I_features, distance=Distance.L2, nnsigma=float(1.0)):
    T_features = tf.convert_to_tensor(value=T_features, dtype=tf.float32)
    I_features = tf.convert_to_tensor(value=I_features, dtype=tf.float32)

    with tf.compat.v1.name_scope('CX'):
        cs_flow = CSFlow.create(I_features, T_features, distance, nnsigma)
        # sum_normalize:
        height_width_axis = [TensorAxis.H, TensorAxis.W]
        # To:
        cs = cs_flow.cs_NHWC
        k_max_NC = tf.reduce_max(input_tensor=cs, axis=height_width_axis)
        CS = tf.reduce_mean(input_tensor=k_max_NC, axis=[1])
        CX_as_loss = 1 - CS
        CX_loss = -tf.math.log(1 - CX_as_loss)
        CX_loss = tf.reduce_mean(input_tensor=CX_loss)
        return CX_loss