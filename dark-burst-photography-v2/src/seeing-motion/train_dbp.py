...
ps = 918
n_burst = 8
in_image = tf.compat.v1.placeholder(tf.float32, [None, 1, ps, ps, 4])
in_image_low = tf.compat.v1.placeholder(tf.float32, [None, 1, ps//2, ps//2, 4])
gt_image = tf.compat.v1.placeholder(tf.float32, [1, ps, ps, 3])
...
loss_sum = tf.compat.v1.summary.scalar('loss',G_loss)
sum_writer = tf.compat.v1.summary.FileWriter('./log',sess.graph)


saver = tf.compat.v1.train.Saver(t_vars)
