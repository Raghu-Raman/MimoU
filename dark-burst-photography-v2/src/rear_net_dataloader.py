...
class Net_DataLoader(object):
    ...
    def random_crop_penang(self, gt_image,input_images):
        cropped_image = tf.image.random_crop(input_images, size=[len(input_images), self.crop_size_h, self.crop_size_w, 3])
        ...
        for jpg in jpgs:
            if (b'arcsoft' in jpg):
                ...
                if (ev_num not in evs.keys()):
                    ...
                    evs[ev_num].append(jpg)
        samsungs
        samsung_path = folder_path + samsungs[0]
        inp_ev_paths = []
        for inp_ev, num_frames in input_evs:
            temp = int(inp_ev)
            inp_ev_paths.extend(evs[temp][0:num_frames])

        inp_ev_paths = [folder_path + i for i in inp_ev_paths]

        return samsung_path, inp_ev_paths
    ...
    def transform_train(self, input, target):
        ...
        input_image_y420 = tf.expand_dims(input_image_y420,axis=2)
        target_image_y420 = tf.expand_dims(target_image_y420,axis=2)
        print("anv_loader,y420",input_image_y420.shape)
        print("anv_loader,uv420",input_image_uv420.shape)
        print("anv_loader,targety420",target_image_y420.shape)
        print("anv_loader,targetuv420",target_image_y420.shape)
    ...
    def anv_testDataLoader(self,input_evs):
        ...
        train_dataset = tf.data.Dataset.from_generator(generator=self.anv_generator_test,args=[folder_paths,input_evs,self.batch_size], output_types=(tf.float32,tf.float32,tf.float32,tf.float32),
                                                       output_shapes=((None,3072,4096,1),(None,3072//2,4096//2,2),
                                                                      (None,3072,4096,3),(None,3072//2,4096//2,6)))
        if(self.strategy):
            train_dataset = train_dataset.batch(self.batch_size * self.strategy.num_replicas_in_sync).prefetch(self.prefetch)
            train_dataset = self.strategy.experimental_distribute_dataset(train_dataset)
        else:
    ...
    def anv_generator_test(self,folder_paths, input_evs,batch_size):
        ...
        while True:
            if i*batch_size >= len(folder_paths):
                ...
                # print(target_batch_y.shape)
                # print(target_batch_uv.shape)
                # print(input_batch_y.shape)
                # print(input_batch_uv.shape)
                yield target_batch_y, target_batch_uv , input_batch_y, input_batch_uv
                i += 1
